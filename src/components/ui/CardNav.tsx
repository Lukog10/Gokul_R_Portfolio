...
export default function CardNav({
    ...
    const allHrefs = ['/projects', '/experience', '/skills', '/achievements', '/resume'];
    const isActive = useMemo(() => {
        return allHrefs.some(href => pathname === href || pathname.startsWith(`${href}/`));
    }, [pathname]);

    const activeLabel = useMemo(() => {
        if (!isActive) return aboutItem.label;

        // Try to match standard sub-links
        ...
        // Fallbacks for layout sidebars
        if (pathname === '/resume' || pathname.startsWith('/resume/')) return "Resume";

        return aboutItem.label;
    }, [isActive, pathname, items, aboutItem]);
        ...
        <div ref={containerRef} className="relative">
            ...
            <AnimatePresence>
                {isExpanded && (
                    ...
                    >
                        <div className={cn(
                            "relative w-[640px] rounded-[1.5rem] border shadow-2xl backdrop-blur-2xl transition-all overflow-hidden",
                            theme === 'dark'
