...
import { GraduationCap, Binary, Clock } from "lucide-react";
    ...
    return (
        <div className="w-full max-w-6xl mx-auto p-4 md:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* The American College Box (Left) - Hover Effect: Translate Y & Blue Glow */}
                <motion.div 
                    initial={isLowPowerMode ? {} : { opacity: 0, y: 20 }}
                    whileInView={isLowPowerMode ? {} : { opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ duration: 0.5 }}
                    className="col-span-1 md:col-span-2 border border-black/10 dark:border-white/10 bg-neutral-100 dark:bg-[#0a0a0a] overflow-hidden relative group flex flex-col md:flex-row min-h-[300px] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(59,130,246,0.3)] hover:border-blue-500/50"
                >
                    <CornerAccents hoverClass="group-hover:border-blue-500 dark:group-hover:border-blue-400" />
                    {/* Text Section (Top) */}
                    <div className="p-8 relative z-10 transition-transform duration-500 group-hover:translate-x-1 md:w-3/5 md:flex-shrink-0">
                        <div className="flex items-center gap-2 mb-4">
                            <span className="text-xs font-bold uppercase tracking-widest text-neutral-500 dark:text-neutral-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">Higher Education • Completed</span>
                        </div>
                        <h3 className="text-3xl font-black text-neutral-900 dark:text-white mb-4">The American College</h3>
                        <p className="text-sm text-neutral-600 dark:text-neutral-400 leading-relaxed">
                            B.Sc. Data Science student at Madurai, Tamil Nadu with a CGPA of 8.5/10.0. Focused on data analysis, machine learning algorithms, database engineering, and dynamic dashboards.
                        </p>
                    </div>

                    {/* Visual Section (Bottom) */}
                    <div className="md:w-2/5 flex items-center justify-center relative p-8 md:mt-0 mt-auto md:border-t-0 md:border-l border-t border-black/10 dark:border-white/10 bg-gradient-to-b md:bg-gradient-to-r from-transparent to-black/5 dark:to-white/5 overflow-hidden">
                        {/* Background Logo Overlay */}
                        <div className="absolute inset-0 bg-gradient-to-t from-blue-950/20 via-black/5 to-transparent transition-opacity duration-500 group-hover:opacity-80" />
                        ...
                        <div className="relative z-10 flex flex-col items-center transition-transform duration-500 group-hover:scale-105">
                            ...
                            <div className="flex flex-wrap gap-2 justify-center mb-4">
                                {["CGPA 8.5", "Data Science", "Madurai, TN"].map(s => (
                            ...
                            <p className="text-[10px] font-mono text-white/90 uppercase tracking-widest bg-black/50 px-2 py-1 rounded backdrop-blur-sm border border-white/10 group-hover:border-blue-500/50 transition-colors">Data Science & Analytics</p>
                        </div>
